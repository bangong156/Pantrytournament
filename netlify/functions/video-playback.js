...
export function createHandler(resolve=dependencies){return async function handler(event){
  ...
  try{
    ...
    const rows=(await repository.eventRows(event_id)).filter(row=>row.tournament_id===tournament_id&&(!match_id||row.match_id===match_id)&&row.status==='live'&&Date.parse(row.hard_expires_at)>Date.now()&&Date.parse(row.lease_expires_at)>Date.now());
