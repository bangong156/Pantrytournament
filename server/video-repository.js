...
export function videoRepository({url,key}){
  ...
  return {
    ...
    async insert(row){return check(await db.from('match_video_sessions').insert(row).select().single());},
    async cas(row,patch){return check(await db.from('match_video_sessions').update({...patch,version:row.version+1}).eq('id',row.id).eq('version',row.version).eq('status',row.status).select().maybeSingle());},
    async eventRows(id){return check(await db.from('match_video_sessions').select('*').eq('event_id',id).neq('status','ended').limit(100));},
    async due(now){return check(await db.from('match_video_sessions').select('*').neq('status','ended').lte('cleanup_after',now).order('cleanup_after').limit(12));},
    async prune(now){check(await db.from('match_video_rate_limits').delete().lt('window_start',new Date(now-86400000).toISOString()));}
  };
}
