...
async function refereeWriteError(tid,token,error){try{if(!await validRefereeSession(tid,token)){clearRefereeSession(tid);await publicTournament(tid);return}}catch{}alert(error.message)}
...
async function refereeConsole(tid,token,validated=null){
 ...
 const [{data:t},{data:g},{data:teams},{data:matches},{data:mlp},{data:slots},standings]=await Promise.all([supabase.from('tournaments').select('*').eq('id',tid).single(),supabase.from('groups').select('*').eq('id',saved.group_id).eq('tournament_id',tid).single(),supabase.from('teams').select('id,name').eq('tournament_id',tid),supabase.from('matches').select('*').eq('tournament_id',tid).eq('group_id',saved.group_id).eq('stage','group').order('match_code'),supabase.from('mlp_configs').select('style,members_per_team').eq('tournament_id',tid).maybeSingle(),supabase.from('mlp_slots').select('id').eq('tournament_id',tid),standingsData(tid)]);
 if(epoch!==renderEpoch)return;
 ...
 app.innerHTML=`<header class="ref-header"><div><button class="ghost" id="leaveRef">← Thoát bàn</button><b>⚖ Bảng ${esc(g?.name||saved.group_name)}</b></div><div class="ref-name">${esc(saved.referee_name)}</div></header><main class="wrap referee-page"><div class="referee-banner"><div><small>REFEREE MODE</small><h1>Bảng ${esc(g?.name||saved.group_name)}</h1><p>${esc(t?.name)} · Chỉ nhập kết quả của bảng này.</p></div><span>${esc(saved.referee_name)}</span></div><div class="ref-layout"><section><div class="ref-match-list">${(matches||[]).map((m,i)=>{const done=m.status==='completed';return `<article class="ref-match ${done?'done':''}"><div class="ref-order"><span>${String(i+1).padStart(2,'0')}</span><small>${esc(m.match_code)}</small></div><div class="ref-teams"><b>${esc(tm[m.team1_id]||'TBD')}</b><span>VS</span><b>${esc(tm[m.team2_id]||'TBD')}</b></div>${isBasic?`<div class="series-result"><b>${m.team1_score??'–'}</b><span>:</span><b>${m.team2_score??'–'}</b></div><button data-ref-mlp="${m.id}" class="${done?'saved-btn':''}">${done?'✓ '+m.team1_score+'–'+m.team2_score:'Nhập game'}</button>`:`<div class="ref-score"><input type="number" min="0" data-ref-s1="${m.id}" data-original="${m.team1_score??''}" value="${m.team1_score??''}"><b>:</b><input type="number" min="0" data-ref-s2="${m.id}" data-original="${m.team2_score??''}" value="${m.team2_score??''}"></div><button data-ref-save="${m.id}" class="${done?'saved-btn':''}">${done?'Sửa kết quả':'Xác nhận'}</button>`}</article>`}).join('')||'<div class="panel">Chưa có trận.</div>'}</div></section><aside class="ref-standing"><h3>BXH LIVE</h3><table><tr><th>#</th><th>Đội</th><th>W</th><th>+/-</th></tr>${st.map((r,i)=>`<tr class="${i<2?'qualify-row':''}"><td>${i+1}</td><td><b>${esc(r.name)}</b></td><td>${r.w}</td><td>${r.diff>0?'+':''}${r.diff}</td></tr>`).join('')}</table></aside></div></main><div id="modal"></div>`;
 document.querySelector('#leaveRef').onclick=()=>{clearRefereeSession(tid);publicTournament(tid)};
 document.querySelectorAll('[data-ref-save]').forEach(b=>b.onclick=async()=>{const id=b.dataset.refSave,a=document.querySelector(`[data-ref-s1="${id}"]`).value,c=document.querySelector(`[data-ref-s2="${id}"]`).value;if(a===''||c==='')return alert('Nhập đủ 2 tỉ số.');if(+a===+c)return alert('Tỉ số không được hòa.');const m=(matches||[]).find(x=>x.id===id);if(!confirm(`Xác nhận kết quả\n${tm[m.team1_id]} ${a} – ${c} ${tm[m.team2_id]}?`))return;b.disabled=true;b.textContent='Đang lưu…';const {error}=await supabase.rpc('referee_submit_score',{p_session_token:token,p_match_id:id,p_team1_score:+a,p_team2_score:+c});if(error){b.disabled=false;return refereeWriteError(tid,token,error)}refereeConsole(tid,token)});
 ...
 startLive(`ref:${tid}`,()=>refereeConsole(tid,token));
}
...
async function dashboard(){stopLive();const epoch=++renderEpoch;const {data:t}=await supabase.from('tournaments').select('*').order('start_date',{ascending:false});if(epoch!==renderEpoch)return;app.innerHTML=`<header><div><b>THE PANTRY</b><span>Tournament Manager</span></div><div>${esc(profile?.full_name)} · ${esc(profile?.role)} <button class="ghost" id="logout">Đăng xuất</button></div></header><main class="wrap"><div class="hero"><div><small>CONTROL CENTER</small><h1>Giải đấu & Minigame</h1><p>Tạo giải, import VĐV, chia bảng và nhập kết quả tại một nơi.</p></div><button id="new">＋ Tạo giải</button></div><div class="cards">${(t||[]).map(x=>`<article class="card" data-id="${x.id}"><div class="pill">${x.event_type==='minigame'?'MINIGAME':'GIẢI ĐẤU'}</div><h3>${esc(x.name)}</h3><p>${x.format==='mlp'?'Đồng đội / MLP':'Đánh đôi'} · ${esc(x.start_date)}</p><strong>${esc(x.status)}</strong></article>`).join('')||'<div class="empty">Chưa có giải nào. Tạo giải đầu tiên để bắt đầu.</div>'}</div></main><div id="modal"></div>`;document.querySelector('#logout').onclick=async()=>{await supabase.auth.signOut();session=null;render()};document.querySelector('#new').onclick=createModal;document.querySelectorAll('.card').forEach(c=>c.onclick=()=>workspace(c.dataset.id))}
...
async function workspace(id,tab='overview'){
  ...
  if(epoch!==renderEpoch)return;
  app.innerHTML=`<header><div><button class="ghost" id="back">← Dashboard</button><b>${esc(t.name)}</b></div><div>${t.format==='mlp'?'MLP':'ĐÁNH ĐÔI'} · ${esc(t.start_date)}</div></header><main class="wrap"><div class="workspace"><aside>${navButton('overview','Tổng quan',tab)}${navButton('teams','VĐV / Đội',tab)}${navButton('groups','Chia bảng',tab)}${navButton('matches','Trận đấu',tab)}${navButton('standings','BXH',tab)}${navButton('referees','Trọng tài',tab)}${navButton('knockout','Knockout',tab)}</aside><section id="workcontent"></section></div></main><div id="modal"></div>`;
...
async function renderRefereeAdmin(tid){
 const epoch=renderEpoch;
 const [{data:groups},{data:codes},{data:sessions},{data:logs},{data:matches}]=await Promise.all([
