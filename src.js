...
async function publicTournament(tid,tab='overview'){
 ...
 const [{data:t},{data:groups},{data:teams},{data:matches},standings,{data:awards},{data:roster,error:rosterError}]=await Promise.all([
  supabase.from('tournaments').select('id,name,event_type,format,start_date,status').eq('id',tid).single(),
  ...
  supabase.from('matches').select('id,group_id,match_code,status,team1_id,team2_id,team1_score,team2_score').eq('tournament_id',tid).eq('stage','group').order('match_code'),
  standingsData(tid),
  supabase.from('tournament_awards').select('placement,placement_slot,team_name,player_names').eq('tournament_id',tid).order('placement').order('placement_slot'),
  supabase.rpc('public_tournament_roster',{p_tournament_id:tid})
 ]);
 if(epoch!==renderEpoch)return;
 ...
 app.innerHTML=`<header class="public-header public-tournament-header"><div><button class="ghost" id="publicBack">← Danh sách giải</button>${pantryLogoMarkup('pantry-logo-small')}<b>${esc(t.name)}</b></div><button id="beReferee">⚖ Tôi là trọng tài</button></header><main class="wrap public-event public-hub"><div class="public-event-head"><div><small>${t.event_type==='minigame'?'MINIGAME':'GIẢI ĐẤU'} · ${t.format==='mlp'?'MLP':'ĐÁNH ĐÔI'}</small><h1>${esc(t.name)}</h1><p>${esc(t.start_date)} · ${groups?.length||0} bảng</p></div>${(matches||[]).some(m=>m.status==='playing')?'<div class="live-badge"><span></span> LIVE</div>':''}</div><nav class="public-hub-tabs" aria-label="Nội dung giải đấu">${tabs.map(([key,label])=>`<button data-public-tab="${key}" class="${tab===key?'public-hub-tab-active':''}" ${tab===key?'aria-current="page"':''}>${label}</button>`).join('')}</nav><div class="public-hub-view">${publicHubContent(tab,context)}</div></main><div id="modal"></div>`;
...
function publicHubContent(tab,c){
 ...
 if(tab==='teams')return `${rosterProblem}<div class="public-hub-team-grid">${teams.map(team=>`<article class="public-hub-team-card"><div class="public-hub-team-title">${publicTeamButton(team.id,team.name)}</div>${rosterError?'':names(team.id).length?`<ol>${names(team.id).map(n=>`<li>${esc(n)}</li>`).join('')}</ol>`:'<p>Chưa có VĐV.</p>'}</article>`).join('')||'<div class="public-hub-empty">Chưa có đội đăng ký.</div>'}</div>`;
