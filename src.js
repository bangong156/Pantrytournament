...
async function refereeConsole(tid,token,validated=null,showList=false){
 ...
 document.querySelectorAll('[data-live-start]').forEach(b=>b.onclick=async()=>{
  ...
  const id=b.dataset.liveStart,target=+(document.querySelector(`[data-ref-target="${id}"]`)?.value||11),winTwo=document.querySelector(`[data-ref-win-two="${id}"]`)?.checked??true;
...
function renderScoreboard(tid,token,state,message=''){
 ...
 document.querySelectorAll('[data-score-team]').forEach(b=>b.onclick=async()=>{
  ...
  renderScoreboard(tid,token,data,data.stale?'Điểm đã đổi trên thiết bị khác. Kiểm tra rồi bấm lại.':'Đã lưu điểm');
 });
